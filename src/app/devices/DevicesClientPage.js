...
export default function DevicesClientPage({ initialDeviceMap, initialError }) {
  ...
  const filtered = useMemo(
    () =>
      deviceMap
        ?.filter((d) => maintainedFilter === null || d.isMaintained === maintainedFilter)
  ...
  useEffect(() => {
    if (maintainedFilter === null) {
      ...
      setVisibleOems(Array.from(filteredOems).sort((a, b) => a?.localeCompare(b)));
    }
      ...
      <div className='flex flex-col items-center justify-center gap-20 xl:gap-24'>
        ...
        >
          <div className='inline-flex flex-col items-baseline gap-2 text-center font-[Prod-bold] text-4xl sm:flex-row sm:text-5xl lg:gap-4 lg:text-6xl'>
            <img className='h-7 sm:h-10 lg:h-12' src="/evolution.svg" alt='' />
