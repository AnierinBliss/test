  ...
  return (
    <>
      <button
        ...
        disabled={!initialChangelog && changelogError?.message === "No changelog available for this branch."}
      >
        Changelog
      </button>
        ...
        {showChangelog && (
          ...
          >
            <div className='relative mx-[2rem] min-h-0 grow rounded-3xl border-4 border-dashed border-[#0060ff] bg-[#0f172a]'>
              ...
              >
                <img src="/menuClose.svg" alt='close' className='size-6' />
              </span>
              <div className='h-full overflow-y-scroll px-10 pt-5 lg:pt-[1rem]'>
                {changelogError ? (
                  <p className='text-red-500'>
                    Error fetching changelog: {changelogError.message || 'Unknown error'}
                  </p>
                ) : initialChangelog ? (
                  ...
                  <p className="text-white">No changelog available for this device and branch.</p>
                )}
              </div>
            </div>
          </motion.div>
